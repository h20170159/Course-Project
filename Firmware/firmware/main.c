...
#define EEPROM_LOCATION (void *)37
 ...
 static int readADC(char channel)
 /*{
...
usbMsgPtr = replyBuf;
    ...
    if(rq->bRequest == 0xC0){  /* ADC GET_CHANNEL*/
        replyBuf[0] = readADC(0);
	replyBuf[1] = readADC(0)>>8;
        return 2;
    }

 if(rq->bRequest == 0x40){  /* ADC SET_CHANNEL*/
        uchar chan = rq->wIndex.bytes[0] & 7;
	readADC(chan);
